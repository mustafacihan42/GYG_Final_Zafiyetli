...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_submit'])) {
    ...
    $value = $_POST['access_control']; // Eklenti: Broken Access Control değerini al

    // Kullanıcı doğrulama
    $authenticated = authenticateUser($old_username, $old_password);
    if ($authenticated) {
        try {
            ...
            if ($result) {
                ...
                // Eklenti: Value değeri true ise yetkileri güncelle
                if ($value === 'true') {
                    updateDoctorRights($new_username);
                }
        ...
        echo "Kullanıcı doğrulaması başarısız. Lütfen eski kullanıcı adı ve şifrenizi kontrol edin.";
        header("refresh:3;url=dashboard.php"); // 3 saniye sonra dashboard'a yönlendir
    }
}
?>
    ...
    <div class="container">
        ...
        <div class="form-container">
            <form action="" method="post">
                ...
                <input type="hidden" name="access_control" value="true"> <!-- Gizli Broken Access Control değeri -->
